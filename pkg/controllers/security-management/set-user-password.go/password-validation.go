...
func PasswordValidation(password, instiCode, appCode, username, moduleName, methodUsed, endpoint string) response.ReturnModel {
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM parameters.system_config_params WHERE config_value = 'true' AND config_code IN('pass_lowc','pass_upc','pass_num','pass_sym') AND config_insti_code = ? AND config_app_code = ?", instiCode, appCode).Scan(&passCharValidation).Error; fetchErr != nil {
		returnMessage := middleware.ResponseData(username, instiCode, appCode, moduleName, funcName, "302", methodUsed, endpoint, []byte(""), []byte(""), "", fetchErr, nil)
		if !returnMessage.Data.IsSuccess {
			return (returnMessage)
		}
