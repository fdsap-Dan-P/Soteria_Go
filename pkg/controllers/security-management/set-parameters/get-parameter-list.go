...
func ParameterList(c *fiber.Ctx) error {
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM parameters.system_config_params WHERE insti_code = ? AND app_code = ?", headerValidationResponse.Insti_code, headerValidationResponse.App_code).Scan(&configParam).Error; fetchErr != nil {
	...
	successMessage := middleware.ResponseData(headerValidationResponse.Username, headerValidationResponse.Insti_code, headerValidationResponse.App_code, moduleName, funcName, "302", methodUsed, endpoint, []byte(""), []byte(""), "", nil, nil)
