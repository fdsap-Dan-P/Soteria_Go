...
func UpdateUserDetails(c *fiber.Ctx) error {
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM public.user_details WHERE (staff_id = ? OR username = ? OR email = ? OR phone_no = ?) AND application_code = ?", userIdentity, userIdentity, userIdentity, userIdentity, validationDetails.App_code).Scan(&UserDetails).Error; fetchErr != nil {
	...
	if userCategory == "staff" {
		...
		if !isStaffIdValidated {
			returnMessage := middleware.ResponseData(validationDetails.Username, validationDetails.Insti_code, validationDetails.App_code, moduleName, funcName, "103", methodUsed, endpoint, newUserRequestByte, []byte(""), "Invalid Employee ID", nil, nil)
			if !returnMessage.Data.IsSuccess {
				return c.JSON(returnMessage)
			}
		}
		...
		if fetchErr := database.DBConn.Raw("SELECT * FROM public.user_details WHERE staff_id = ? AND user_id != ? AND application_code = ? AND institution_code = ?", newUserRequest.Staff_id, userIdToBeUpdated, validationDetails.App_code, validationDetails.Insti_code).Scan(&UserDetailsChecker).Error; fetchErr != nil {
	...
	// validate if username already exists
	if fetchErr := database.DBConn.Raw("SELECT * FROM public.user_details WHERE username = ? AND user_id != ? AND application_code = ? AND institution_code = ?", newUserRequest.Username, userIdToBeUpdated, validationDetails.App_code, validationDetails.Insti_code).Scan(&UserDetailsChecker).Error; fetchErr != nil {
		returnMessage := middleware.ResponseData(validationDetails.Username, validationDetails.Insti_code, validationDetails.App_code, moduleName, funcName, "302", methodUsed, endpoint, newUserRequestByte, []byte(""), "", fetchErr, fetchErr.Error())
		if !returnMessage.Data.IsSuccess {
			return c.JSON(returnMessage)
		}
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM public.user_details WHERE phone_no = ? AND user_id != ? AND application_code = ? AND institution_code = ?", newUserRequest.Phone_no, userIdToBeUpdated, validationDetails.App_code, validationDetails.Insti_code).Scan(&UserDetailsChecker).Error; fetchErr != nil {
		returnMessage := middleware.ResponseData(validationDetails.Username, validationDetails.Insti_code, validationDetails.App_code, moduleName, funcName, "302", methodUsed, endpoint, newUserRequestByte, []byte(""), "", fetchErr, fetchErr.Error())
		if !returnMessage.Data.IsSuccess {
			return c.JSON(returnMessage)
		}
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM public.user_details WHERE email = ? AND user_id != ? AND application_code = ? AND institution_code = ?", newUserRequest.Email, userIdToBeUpdated, validationDetails.App_code, validationDetails.Insti_code).Scan(&UserDetailsChecker).Error; fetchErr != nil {
		returnMessage := middleware.ResponseData(validationDetails.Username, validationDetails.Insti_code, validationDetails.App_code, moduleName, funcName, "302", methodUsed, endpoint, newUserRequestByte, []byte(""), "", fetchErr, fetchErr.Error())
		if !returnMessage.Data.IsSuccess {
			return c.JSON(returnMessage)
		}
	...
	if fetchErr := database.DBConn.Raw("SELECT * FROM offices_mapping.institutions WHERE institution_code = ?", newUserRequest.Institution_code).Scan(&institutionDetails).Error; fetchErr != nil {
